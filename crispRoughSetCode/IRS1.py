            ...
            for a in self.reduct_attr:
                ...
                for attr in self.reduct_attr:
                    if attr == a:
                        ...
                        dis_exclude_a = dis_exclude_a.union(self.dis_all[attr])
        ...
        for rule in self.all_rule:
           ...
           for pair in condition:
               if newX[pair[0]] != pair[1]:
                   ...
                   break
           if match:
               match_rule.append((rule[0],rule[1],self.all_rule[rule][0],self.all_rule[rule][1]))
        if len(match_rule) == 1:
           return match_rule[0][1]
        elif len(match_rule) > 1:
           max_cover = 0
           final_decision = None
           for rule in match_rule:
...
class ISwRS:## vote fuzzy rough set
    ...
    def fit(self,X,Y):
        ...
        if X.shape[0]%self.batch_size == 0:
            ...
            for i in range(child_num):
                ...
                self.reduct_hist.append(irs.reduct_attr)
                for rule in irs.fix_rule:
                    if (rule[0],rule[1]) in self.all_fix_rule:
                        current_coverage = self.all_fix_rule[(rule[0],rule[1])]
                        new_coverage = (current_coverage[0]+rule[2],(current_coverage[0]*current_coverage[1]+rule[2]*rule[3])/(rule[2]+current_coverage[0]))
                        self.all_fix_rule[(rule[0],rule[1])] = new_coverage
                for rule in irs.possible_rule:
                    if (rule[0],rule[1]) in self.all_fix_rule:
                        current_coverage = self.all_fix_rule[(rule[0],rule[1])]
                        new_coverage = (current_coverage[0]+rule[2],(current_coverage[0]*current_coverage[1]+rule[2]*rule[3])/(rule[2]+current_coverage[0]))
    ...
    def update_group(self, newX, newY):
        num_to_fill_batch = self.batch_size-len(self.left_Y)
        if newX.shape[0] < num_to_fill_batch:
            self.left_X.append(newX.tolist())
            self.left_Y.append(newY.tolist())
        elif newX.shape[0] == num_to_fill_batch:
            self.left_X.append(newX.tolist())
            self.left_Y.append(newY.tolist())
            irs = I_RS()
            ...
            to_fill_X = newX[:num_to_fill_batch]
            to_fill_Y = newY[:num_to_fill_batch]
            self.left_X.append(to_fill_X.tolist())
            self.left_Y.append(to_fill_Y.tolist())
            irs = I_RS()
            irs.fit(np.array(self.left_X),np.array(self.left_Y))
            self.reduct_hist.append(irs.reduct_attr)
            for rule in irs.fix_rule:
        ...
        for rule in self.all_rule:
           ...
           for pair in condition:
               if newX[pair[0]] != pair[1]:
                   ...
                   break
           if match:
               match_rule.append((rule[0],rule[1],self.all_rules[rule][0],self.all_rules[rule][1]))
        if len(match_rule) == 1:
           return match_rule[0][1]
        elif len(match_rule) > 1:
           max_cover = 0
