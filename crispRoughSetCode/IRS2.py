...
class VIRS:
    ...
    def predict(self,newX):
        predictions = {}
        for i in range(self.child_num):
            prediction,support = self.child_list[i].predict(newX)
            if prediction in predictions:
                predictions[prediction][0] = predictions[prediction][0] + 1
                predictions[prediction][1] = predictions[prediction][1] + support
            else:
                predictions[prediction] = [1,support]
        max_key = []
        max_support = 0
        ...
        if len(max_key) == 1:
            ...
            for i in range(len(max_key)):
                if predictions[max_key[i]][1] > max_support:
                    max_support = predictions[max_key[i]][1]
