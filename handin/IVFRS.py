import FRS as mrs
import numpy as np
class IVFRS:## vote fuzzy rough set
    ...
    def fit(self,X,Y,cat= []):
        ...
        for i in range(child_num):
            ...
            self.child_list.append(irs)# add the child to the list
        if X.shape[0]%self.batch_size != 0:# add the left data to wait for more data
    ...
    def predict(self,newX):
        decision_dict = {}
        for child in self.child_list:#for each child, make a prediction on newX
            decision,cover = child.predict(newX)
            if decision in decision_dict:
                decision_dict[decision][0] = decision_dict[decision][0] + 1# add vote by one
                decision_dict[decision][1] = decision_dict[decision][1] + cover # add support
            else:
                decision_dict[decision] = [1,cover]#initialize
        candidate_decision = []
        max_vote = 0
        for decision in decision_dict: # find the decision with maximum vote
        ...
        if len(candidate_decision) == 1: # if no tie, return the maximum
            return candidate_decision[0][0]
        else: # if tie, return the one with highest support
            max_cover = 0
            decision = None
