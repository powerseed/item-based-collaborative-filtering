import FRS as mrs
import numpy as np
class SwVFRS:## Sliding window vote rough set
    ...
    def fit(self,X,Y, cat = []):
        ...
        if child_num <= self.window_size:
            pass
        else:# start from the batch we will eventually keep
        ...
        if X.shape[0]%self.batch_size != 0:#add the rest of data
    ...
    def update(self,newX,newY):
        ...
        if len(self.rule_lists) == self.batch_size:
            ...
            self.left_X = []
            self.left_Y = []
            if len(self.child_list) > self.window_size:#exceed? remove the first child
